...
with Profiler() as p:
    ...
    etalon = 'ru'
Test.Add(service, service + sdef, 'normal 1', test, etalon)

with Profiler() as p:
    test = Yandex.FindLang(' Английский ')
    etalon = 'en'
Test.Add(service, service + sdef, 'normal 2', test, etalon)
...
Test.Add(service, service + sdef, 'normal 1', test, True)
Test.Add(service, service + sdef, 'normal 2', Fixer.region, etalon)

with Profiler() as p:
    Fixer.region = ''
    test = Yandex.isStational(' МСК ')
    etalon = 'Москва и Московская область'
Test.Add(service, service + sdef, 'normal 3', Fixer.region, etalon)

with Profiler() as p:
    Fixer.region = ''
    test = Yandex.isStational('Элийский')
    etalon = ''
Test.Add(service, service + sdef, 'unreal', Fixer.region, etalon)
...
Регион:  -> Russia"""
Test.Add(service, service2 + sdef, 'normal', test, etalon)
...
Test.Add(service, service2 + sdef, 'unreal', test, etalon)

with Profiler() as p:
    test = Ya.Catalog(0)
    etalon = "#bug: 'int' object has no attribute 'strip'"
Test.Add(service, service2 + sdef, 'crush', test, etalon)
