...
import config
from fixer import log, err_log, load_byte, get_params
# import Bot
import apiai
import json
import random
...
def task(text: str) -> str:
    """Сервис заданий: #task: type | time | times | Notice/Service"""
    log(text)
    import Notification
    ...
    etime = datetime.strptime(str(date.today())+' '+params[1], time_format) + timedelta(hours=Fixer.TimeZone)
    ...
    #print(m)
    Fixer.save_byte(Notification.Tasks, 'Tasks') # Сохранение задач
...
def correction(text):
    ...
    Fixer.NewDialogs[Fixer.strCleaner(Fixer.Query)] = Fixer.get_params(text, ';')
